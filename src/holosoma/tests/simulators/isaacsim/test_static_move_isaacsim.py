...
Marked ``isaacsim`` so the IsaacSim CI job (``-m "isaacsim"``) collects it; the IsaacGym job
(``-m "not isaacsim"``) skips it. ``importorskip("isaaclab")``/CUDA-gated so direct/unfiltered
